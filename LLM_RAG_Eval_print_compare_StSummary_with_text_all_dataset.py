...
Albert Einstein was born in 1879. VERDICT: PASSED
Albert Einstein was born in Germany. VERDICT: FAILED
...
CSV_FILE = "./save_csv/evaluation_results_noStText_text_gemma29b.csv"
# use RELATION data
RELATION = False
PKL_FILE_SAVE = "./save/St_compare_Sum_with_text_with_faith_gemma29b.pkl"
...
def extract_statements(text: str) -> List[str]:
    """Extract statements from the given text using Ollama."""
    print_step(1, "Extracting Statements")

    prompt = STATEMENT_EXTRACTION_PROMPT.format(text=text)
...
def extract_statements_chunk(text: str, sentences_per_chunk: int = 10000) -> List[str]:
...
def calculate_metrics(labels: Dict[str, List[str]], metric: str) -> Dict[str, float]:
    ...
    if metric == "correctness":
        ...
        results = {"faithfulness_precision": precision}
...
def calculate_metrics_all(labels: Dict[str, List[str]]) -> tuple[dict[str, float | int], dict[str, float | int]]:
    print_step(4, f"Calculating Metrics")
    tp = len(labels["TP"])
    fp = len(labels["FP"])
    fn = len(labels["FN"])
    precision = tp / (tp + fp) if (tp + fp) > 0 else 0
    ...
    passed = len(labels["PASSED"])
    failed = len(labels["FAILED"])
    faithfulness_precision = passed / (passed + failed) if (passed + failed) > 0 else 0
    faithfulness_f1 = 2*(faithfulness_precision * recall) / (faithfulness_precision + recall) if (faithfulness_precision + recall) > 0 else 0
    results_corr = {"precision": precision, "recall": recall, "f1": f1}
    results_faith = { "faithfulness_precision": faithfulness_precision, "faithfulness_f1": faithfulness_f1 }
    results = {**results_corr, **results_faith}
    print(colored("Calculated Metrics:", 'green'))
    for k, v in results.items():
        print(f"{k}: {v:.4f}")
    return results_corr, results_faith
...
def evaluate_summary(summary: str, original_text: str) -> Dict[str, Dict[str, float]]:
    ...
    correctness_labels = label_statements(summary_statements=summary_statements,
                                          original_statements=[original_text], original_text=original_text,
                                          metric="correctness")
    #correctness_metrics = calculate_metrics(correctness_labels, "correctness")

    faithfulness_labels = label_statements(summary_statements, [original_text], original_text, "faithfulness")
    #faithfulness_metrics = calculate_metrics(faithfulness_labels, "faithfulness")

    correctness_metrics, faithfulness_metrics = calculate_metrics_all({**correctness_labels, **faithfulness_labels})
    results = {
        "original_statements": [original_text],
    ...
    visualize_results(faithfulness_metrics, "faithfulness")
    return results
...
def save_to_csv(all_results, filename=CSV_FILE):
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for i, result in enumerate(all_results):
            #print(result)
            for input_type in ['original', 'simplified']:#, 'relation']:
...
if __name__ == "__main__":
    ...
    for sample_data in data:
        ...
        for input_type in input_types:
            ...
            results[f'{input_type}_labeling_statements'] = results_all[input_type]["faithfulness_labels"]
            results[f'{input_type}_faithfulness'] = results_all[input_type]["faithfulness"]
